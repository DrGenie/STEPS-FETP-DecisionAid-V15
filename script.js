...
/* ===========================
   Status quo framing (copyable)
   =========================== */

const STATUS_QUO_PLAIN_TEXT = `STATUS QUO IN INDIA (for policy note)

Benchmark: around 1 trained field epidemiologist per 200,000 population (IHR / Global Field Epidemiology Roadmap framing). With a population of roughly 1.3 billion, this implies a long-run need of about 6,500 Intermediate/Advanced field epidemiologists.

Supporting point: the CDC India country profile notes that India needs over 7,000 epidemiologists to meet IHR targets.

Current stock used in STEPS (programme estimate): around 3,300 trained across EIS and FETP tiers. This implies a gap of about 3,200 Intermediate/Advanced-equivalent graduates needed by 2030.

India’s existing three-tier cascade under NCDC stewardship:
- Frontline: ~3 months
- Intermediate: ~12 months
- Advanced / EIS: 2 years

India’s EIS was established in 2012 as a 2-year, mentorship-based in-service programme, complemented by Intermediate and Frontline training.

As of 2024, India has multiple hubs and trains a few hundred participants annually, but this remains below the implied annual flow (about 450–500 Intermediate/Advanced graduates per year) needed to close the target by 2030.

Status quo summary: India has a strong architecture and experience base, but insufficient volume and uneven distribution, especially at Intermediate and Advanced tiers.`;
...
/* ===========================
   Cohorts ↔ horizon ↔ sites capacity logic
   =========================== */

function _readSitesForCohortCapacity() {
  const sitesEl = document.getElementById("available-training-sites");
  const raw = sitesEl ? String(sitesEl.value || "").trim() : "";
  if (!raw) return 1;
  const v = Number(raw);
...
function _durationMonthsForTier(tierKey) {
  const key = String(tierKey || "frontline").toLowerCase();
  const d = (TIER_MONTHS && TIER_MONTHS[key]) ? Number(TIER_MONTHS[key]) : 12;
  return isFinite(d) && d > 0 ? d : 12;
}

function _syncPlanningHorizonInputs(newYears) {
  const ids = ["planning-horizon-config", "adv-planning-horizon", "planning-horizon"];
  ids.forEach((id) => {
    const el = document.getElementById(id);
    if (el) el.value = String(newYears);
  });
}

function _writeCapacityFeedback(maxCohorts) {
  const feedbackEl = document.getElementById("cohort-capacity-feedback");
  if (feedbackEl) {
    feedbackEl.textContent = `Max feasible cohorts in horizon given sites: ${maxCohorts}`;
  }
}

function _flashCapacityWarning(message) {
  const warnEl = document.getElementById("cohort-capacity-warning");
  if (!warnEl) return;
  warnEl.textContent = message || "";
  if (!message) return;
  clearTimeout(warnEl._clearTimer);
  warnEl._clearTimer = setTimeout(() => {
    warnEl.textContent = "";
  }, 3200);
}

function updateCohortCapacityConstraints(options = {}) {
  const { clamp = false, showWarning = false } = options;
  const tierEl = document.getElementById("program-tier");
  const cohortsEl = document.getElementById("cohorts");
  if (!tierEl || !cohortsEl) return;

  const tierKey = String(tierEl.value || "frontline").toLowerCase();
  const durationMonths = _durationMonthsForTier(tierKey);
  ...
  // Ensure horizon can accommodate at least one full cohort for the selected tier.
  const minHorizonYears = Math.max(1, Math.ceil(durationMonths / 12));
  if (planningHorizonYears < minHorizonYears) {
    planningHorizonYears = minHorizonYears;
    appState.epiSettings.general.planningHorizonYears = planningHorizonYears;
    _syncPlanningHorizonInputs(planningHorizonYears);
    if (showWarning) {
      _flashCapacityWarning(
        `Planning horizon increased to ${planningHorizonYears} year(s) to cover the ${durationMonths}-month ${labelForTier(tierKey)} programme.`
      );
    }
  }

  const sites = _readSitesForCohortCapacity();
  const horizonMonths = planningHorizonYears * 12;
  let maxCohorts = Math.floor((horizonMonths / durationMonths) * sites);
  if (!isFinite(maxCohorts) || maxCohorts < 1) maxCohorts = 1;

  cohortsEl.max = String(maxCohorts);
  _writeCapacityFeedback(maxCohorts);
  ...
  if (clamp) {
    ...
    if (current > maxCohorts) {
      ...
      if (showWarning) {
        _flashCapacityWarning(`Cohorts reduced to ${maxCohorts} to fit the planning horizon and available training sites.`);
      }
    }
  }
}

function initCohortCapacityConstraints() {
  const tierEl = document.getElementById("program-tier");
  const cohortsEl = document.getElementById("cohorts");
  const horizonCfgEl = document.getElementById("planning-horizon-config");
  const horizonAdvEl = document.getElementById("adv-planning-horizon");
  const sitesEl = document.getElementById("available-training-sites");

  if (tierEl) tierEl.addEventListener("change", () => updateCohortCapacityConstraints({ clamp: true, showWarning: true }));
  if (sitesEl) sitesEl.addEventListener("input", () => updateCohortCapacityConstraints({ clamp: true, showWarning: true }));
  if (cohortsEl) cohortsEl.addEventListener("input", () => updateCohortCapacityConstraints({ clamp: true, showWarning: true }));

  if (horizonCfgEl) {
    horizonCfgEl.addEventListener("input", () => {
      if (horizonAdvEl) horizonAdvEl.value = horizonCfgEl.value;
      updateCohortCapacityConstraints({ clamp: true, showWarning: true });
    });
  }
  if (horizonAdvEl) {
    horizonAdvEl.addEventListener("input", () => {
      if (horizonCfgEl) horizonCfgEl.value = horizonAdvEl.value;
      updateCohortCapacityConstraints({ clamp: true, showWarning: true });
    });
  }

  // Initial render (quiet).
  updateCohortCapacityConstraints({ clamp: true, showWarning: false });
}

function getConfigFromForm() {
  // Enforce capacity constraints between tier duration, planning horizon and training sites.
  // This updates the cohorts input max, provides inline feedback, and clamps invalid values.
  updateCohortCapacityConstraints({ clamp: true, showWarning: true });
...
function initEventHandlers() {
  ...
  // Live capacity constraints linking cohorts ↔ horizon ↔ tier duration ↔ training sites.
  initCohortCapacityConstraints();
...
/* ===========================
   Status quo framing copy helper
   =========================== */

function ensureCopyTextModal() {
  let modal = document.getElementById("copy-text-modal");
  if (modal) return modal;

  modal = document.createElement("div");
  modal.id = "copy-text-modal";
  modal.className = "modal hidden";
  modal.innerHTML = `
    <div class="modal-content" role="dialog" aria-modal="true" aria-labelledby="copy-text-modal-title">
      <button class="modal-close" aria-label="Close">&times;</button>
      <div class="modal-header">
        <h3 id="copy-text-modal-title">Copy text</h3>
      </div>
      <div class="modal-body">
        <p class="hint" style="margin-bottom:8px;">Select the text below and copy it into your policy note template.</p>
        <textarea id="copy-text-modal-area" rows="14" style="width:100%;"></textarea>
      </div>
    </div>
  `;
...
function openCopyTextModal(title, text) {
  const modal = ensureCopyTextModal();
  const titleEl = modal.querySelector("#copy-text-modal-title");
  const area = modal.querySelector("#copy-text-modal-area");
  if (titleEl) titleEl.textContent = title || "Copy text";
  if (area) {
...
async function tryCopyPlainText(text) {
  try {
...
function initStatusQuoCopyButton() {
  const btn = document.getElementById("copy-status-quo");
  if (!btn) return;
  btn.addEventListener("click", async () => {
    const text = String(STATUS_QUO_PLAIN_TEXT || "").trim();
    if (!text) return;

    const ok = await tryCopyPlainText(text);
    if (ok) {
      showToast("Status quo framing copied to clipboard.", "success");
      return;
    }

    openCopyTextModal("Status quo in India (plain text)", text);
    showToast("Clipboard unavailable. Use the modal to copy the text.", "warning");
  });
}
